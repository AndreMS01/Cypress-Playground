describe('Cypress Playground', () => {
  ...
  it('Types in an input which "signs" a form, then asserts it is signed', () => {
    cy.get('#signature-textarea').type('André');
    cy.contains('#signature', 'André').should('be.visible');
  })

  it('Types in the signature field, checks the checkbox to see the preview, then unchecks it', () => {
    cy.get('#signature-textarea-with-checkbox').type('André');
    cy.get('#signature-checkbox').check();
    cy.contains('#signature-triggered-by-check', 'André').should('be.visible');
    cy.get('#signature-checkbox').uncheck();
    cy.contains('#signature-triggered-by-check', 'André').should('not.exist');  
  })
  ...
  it('Clicks a button and simulates an Network failure', () => {
  ...
  Cypress._.times(10, index => {
    it(`Selects ${index + 1} out of 10`, () => {
      cy.get('input[type="range"]').invoke('val', index + 1).trigger('change');
      cy.contains('p', `You're on level: ${index + 1}`).should('be.visible');
    })
  })

  it('Selects a date and asserts the correct date has been displayed', () => {
    cy.get('input[type="date"]').type('2025-04-04').blur();
    cy.contains('p', "The date you've selected is: 2025-04-04").should('be.visible');
  })
  ...
  it('Types a password based on a protected variable without leaking it', () => {
    ...
    cy.get('#password-input input[type="text"]').should('be.visible').and('have.value', Cypress.env('password'), { log: false });
  ...
  it('Freezes the browser clock and asserts the frozen date is desplayed', () => {
    cy.contains('p', 'Current date: 2025-03-31').should('be.visible');
  })

  it('Copies the code, types it, submits it, then asserts the success message', () => {
    cy.get('#timestamp').then(element => {const code = element[0].innerText
